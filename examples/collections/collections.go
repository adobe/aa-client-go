...
func main() {
	...
	// Get collections
	collections, err := client.Collections.GetAll("", "", 100, 0, []string{"all"})
	if err != nil {
		log.Fatal(err)
	}

	utils.PrintJSON(collections)

	// Get collection
	collection, err := client.Collections.GetByID("<ID>", []string{})
	if err != nil {
		log.Fatal(err)
	}

	utils.PrintJSON(collection)
}
